...
summary: >-
  ...
  为了解决 CI 中间产物检测无法覆盖源码、检出问题定位成本高等问题，实现了 CI 增量源码检测，基于 git diff 获取每个 MR
  对应的所有新增源码，进行安全检查与合码管控。


  CD 产物检测基于 BDAnalysis 引擎实现了调用链的生成，弥补了可能存在的漏放风险，同时采用 dex 扫描替代 smali 扫描，将检测平均耗时从
  175s 降低至 32s，大大提升了检测工具的精度与速度。
---
