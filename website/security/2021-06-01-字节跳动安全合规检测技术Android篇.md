...
summary: >-
  业务安全合规检测嵌入到 CI/CD
  阶段，通过分析检查新增代码、合码管控和问题溯源，以及对构建产物进行发版管控，防止隐私和合规问题进入线上环境，避免安全合规风险。随着技术的发展，检测流程不断改进，包括引入
  CI 增量源码检测，实现检出问题的自动精准定位和问题聚合；以及在 CD 产物检测中采用 BDAnalysis 引擎生成调用链，弥补潜在漏放风险，并通过
  dex 扫描替代 smali 扫描，大幅提升检测效率。
---
