...
summary: >-
  业务安全合规检测通过 CI/CD
  阶段卡口，针对新增代码进行分析检查、合码管控、问题溯源，针对构建产物进行发版管控，避免隐私、合规相关问题被带到线上引发安全合规风险。


  为了解决 CI 中间产物检测无法覆盖源码相关检测、检出问题定位成本高等问题，实现了一套 CI 增量源码检测。该检测基于 git diff 获取每个 MR
  对应的所有新增源码，进行安全检查与合码管控。


  CD 产物检测方面，基于 BDAnalysis 引擎实现了调用链的生成，弥补了可能存在的漏放风险。同时，将 CD 产物检测的平均耗时从 175s 降低至
  32s。


  这些改进提高了检出问题的消费效率，降低了定位解决成本，提升了检测工具的精度与速度。
---
