...
summary: >-
  业务安全合规检测通过 CI/CD
  阶段的卡口，对新增代码进行分析检查、合码管控和问题溯源。针对构建产物进行发版管控，避免隐私和合规问题被带到线上，引发安全合规风险。


  为了解决 CI 中间产物检测无法覆盖源码和检出问题定位成本高等问题，引入了 CI 增量源码检测。它可以覆盖 Android/iOS
  双端的源码检测，并实现问题自动精准定位和问题聚合，从而提高问题的消费效率和降低定位解决成本。


  在 CD 产物检测方面，基于 BDAnalysis 引擎实现了调用链的生成，弥补了可能存在的漏放风险。同时，通过 dex 扫描替代 smali 扫描，将
  CD 产物检测的平均耗时从 175 秒降低到 32 秒，大大提升了检测工具的精度和速度。
---
