...
summary: >-
  **摘要**
  ...
  容器安全是一个复杂且多方面的领域，其中安全隔离通常涉及多项内核技术和特性。随着云原生技术的快速发展，越来越多的容器运行时组件在新版本中默认配置
  AppArmor 策略，这使得传统的容器逃逸技术逐渐失效。本文探讨了内核特性 eBPF 对容器安全性的挑战，并展示了如何利用 eBPF 绕过
  AppArmor 限制，从容器逃逸到宿主机，并进一步利用 Static Pod 实现远程代码执行。


  **eBPF 简介**


  eBPF 是传统 BPF 的继任者，允许在内核中执行代码，而无需加载内核模块。它可以用于安全、跟踪、性能分析和网络数据包处理。


  **新的弱点**


  AppArmor 使用限制 mount 来进一步限制容器，但 eBPF 可以使用诸如 `kprobe` 和 `tracepoint`
  等跟踪技术来绕过此限制。此外，eBPF 还可以修改用户空间内存和挂钩网络数据，从而为编写 rootkit 提供捷径。


  **容器逃逸分析**


  要使用 eBPF，需要一些权限和挂载伪文件系统。eBPF program 作为附加在内核特定 hook point 的应用，在加载时不会考虑被 hook
  的进程所在的 namespace 或 cgroup。


  **vixie-cron 分析**


  vixie-cron 是一个常见的 cron 实现，其处理系统 crontab 的关键函数会先获取一些文件和目录的文件信息。通过 hook `stat`
  系统调用返回，可以修改返回的 `struct stat` buf 里的数据，从而让 cron 立即处理 `/etc/crontab`。


  **eBPF program 编写**


  本文使用 libbpf 编写 eBPF 程序。eBPF 程序可以 hook 进入和退出系统调用，并修改用户态空间的返回数据。


  **RLIMIT 限制绕过**


  Linux kernel 限制 eBPF 程序的内存占用，但在加载和卸载 eBPF 程序时，计算占用内存的字段位于 `user_struct` 的
  `locked_vm` 字段。通过创建一个不同的 user，可以绕过此限制。


  **AFW 到 RCE 新方法**


  Static Pod 是 Kubernetes 中的一种特殊 Pod，由 kubelet 管理。它具有几个优势，包括仅依赖
  kubelet、配置目录固定、执行间隔比 Cron 更短、进程配置更灵活以及检测新文件或文件变化的逻辑更通用。通过劫持 kubelet，可以利用 eBPF
  绕过 AppArmor 限制，从容器逃逸到宿主机，并利用 Static Pod 实现远程代码执行。


  **防御措施**


  * 避免在容器内赋予 `CAP_SYS_ADMIN` 和 `CAP_BPF` 权限

  * 使用 `seccomp` 限制 `bpf` 系统调用

  * 监控系统 `bpf` 调用和加载 eBPF 程序、map 的情况
---
