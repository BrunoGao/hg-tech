...
summary: >-
  **摘要**
  ...
  容器安全面临着来自 eBPF 的新挑战，eBPF 是一种内核特性，允许在内核中执行代码而无需加载内核模块。攻击者可以利用 eBPF 绕过安全隔离机制，包括
  AppArmor、Namespace、Capabilities 和 Cgroup。本文重点探讨了 eBPF 对容器安全的影响，并展示了如何使用 eBPF
  逃逸容器并执行远程代码。此外，本文还提出了利用 Static Pod 作为云原生攻击面的新方法，并讨论了防御措施。


  **eBPF 简介**


  eBPF 是 BPF 的后继者，它允许在内核中执行代码，而无需加载内核模块。它可以用于安全、跟踪、性能分析和网络数据包处理等领域。


  **新的弱点**


  传统的容器逃逸技术依赖于修改文件系统，但 eBPF 提供了一种不需要与伪文件系统交互的方法。通过使用 eBPF，攻击者可以修改用户空间内存，包括 hook
  网络数据，从而为编写 Rootkit 提供捷径。


  **容器逃逸分析**


  要使用 eBPF，需要一些权限和挂载伪文件系统。eBPF kprobe 和 tracepoint 的使用条件如下：
  ...
  | bpf 系统调用 | 拥有 CAP_SYS_ADMIN；内核 5.8 开始拥有 CAP_SYS_ADMIN 或 CAP_BPF |

  | 无特权 bpf - "socket filter" like | kernel.unprivileged_bpf_disabled 为 0
  或拥有上述权限 |

  | perf_event_open 系统调用 | 拥有 CAP_SYS_ADMIN；内核 5.8 开始拥有 CAP_SYS_ADMIN 或
  CAP_PERFMON |

  | kprobe | 需要使用 tracefs；内核 4.17 后可用 perf_event_open 创建 |

  | tracepoint | 需要使用 tracefs |

  | raw_tracepoint | 内核 4.17 后通过 bpf 调用 BPF_RAW_TRACEPOINT_OPEN 即可 |


  **eBPF program 编写**


  本文使用 libbpf 库编写 eBPF 程序。程序包括：


  * **raw_tracepoint/sys_enter**：attach 到进入系统调用时

  * **raw_tracepoint/sys_exit**：attach 到退出系统调用时


  **RLIMIT 限制绕过**


  内核限制 eBPF 程序的内存占用，但在某些情况下，攻击者可以通过创建新的用户来绕过此限制。


  **AFW 到 RCE 新方法**


  本文提出了利用 Static Pod 作为云原生攻击面的新方法。Static Pod 仅依赖于 kubelet，配置目录固定，执行间隔比 Cron
  更短，并且检测新文件或文件变化的逻辑更通用。


  **eBPF 劫持 kubelet 进行逃逸**


  攻击者可以通过 hook `openat`、`read` 和 `close` 系统调用来劫持 kubelet。通过修改 `read` 返回值，攻击者可以覆盖
  Static Pod YAML 文件内容并执行远程代码。


  **防御措施**


  * 限制容器的 CAP_SYS_ADMIN 和 CAP_BPF 权限

  * 使用 seccomp 限制 bpf 系统调用

  * 监控系统 bpf 调用和加载 eBPF 程序
---
