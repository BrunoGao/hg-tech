...
summary: |-
  **应用场景**
  在高并发系统中，限流熔断机制可限制请求数量，避免系统超负荷。

  **常用限流算法**
  * **计数器：**直接计数请求数量，超过上限则拒绝。
  * **滑动窗口：**将时间间隔划分为更小的窗口，每个窗口独立计数。
  * **漏桶：**请求进入固定容量的队列，队列满则拒绝。
  * **令牌桶：**请求获取令牌，令牌数量有限，获取不到则拒绝。

  **限流方案**
  * **容器限流：**通过配置容器（如Tomcat、Nginx）的最大线程数或并发连接数进行限流。
  * **服务端限流：**使用Java中的Semaphore、RateLimiter或Hystrix等工具在服务端实现限流。
  * **分布式限流：**通过中心系统统一管理流量配额，实现分布式环境下的限流。
---
