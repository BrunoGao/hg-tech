...
authors: 京东云开发者
summary: >-
  互联网业务经常面临高并发场景，导致系统不堪重负。限流熔断机制通过限制请求数量和快速失败来保证系统稳定性，防止雪崩效应。


  常用的限流算法包括计数器、漏桶和令牌桶。其中，计数器算法简单暴力，但易产生突刺现象；漏桶算法提供冗余能力，但消费能力固定；令牌桶算法更适合突发流量。


  限流方案包括容器限流和服务端限流。容器限流通过控制连接数或速率来进行限流；服务端限流则使用Semaphore、RateLimiter、Hystrix或Sentinel等工具。


  分布式限流方案通过中心系统管控流量配额，可以实现对共用资源的统一流量限制。常见的实现方式包括使用Tair或Redis的incr方法、lua脚本或令牌桶算法。
---
