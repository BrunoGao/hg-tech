...
authors: 京东云开发者
summary: |-
  互联网应用场景中，高并发导致系统性能下降，需要限流熔断机制来保证系统稳定性。常用限流算法包括计数器、漏桶和令牌桶。

  容器限流方案有Tomcat和Nginx，服务端限流方案有Semaphore、RateLimiter、Hystrix和Sentinel。

  分布式下限流方案采用中心系统流量管控，如Tair和Redis通过自增方法或Lua脚本实现简单窗口或令牌桶算法。
---
