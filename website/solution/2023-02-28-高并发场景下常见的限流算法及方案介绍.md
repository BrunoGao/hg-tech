...
summary: |-
  **应用场景**
  ...
  高并发场景下，系统资源有限，需要限制请求量以保证系统稳定。

  **常用限流算法**

  * **计数器：**直接计数，简单但可能出现突刺现象。
  * **滑动窗口：**将时间划分为窗口，每个窗口独立计数，解决突刺现象。
  * **漏桶：**固定容量的桶，请求到达后放入队列，队列满则拒绝请求。
  * **令牌桶：**固定容量的桶，以固定速率往桶内添加令牌，请求消耗令牌，令牌不足则拒绝请求。
