---
title: "基于 InternLM 和 LangChain 搭建知识库助手"
...
```bash
pip install langchain==0.0.292传感器485实时数据通讯协议
1、主机到从机：
功能描述：请求数据
命令格式：
0xFe,0xFe,0x68,A0,Cmd,Len,Data,CS,0x16
命令解释：
        0xFE,0xFE为同步头，无效数据
        0x68表示有效数据开始传输
A0表示从机地址
...
＝0x91：历史数据，有后续数据包。
        Len表示数据包的长度，这里为0x01
        Data为数据包内容，这里固定为0x34
        CS为校验和，计算0x68到Data部分的校验和(累加和取低字节)01
        0x16为结束标志 
样例（主机向1号从机发送上传实时数据请求）：
0xFE,0xFE,0x68,0x01, 0x80,0x01,0x34,0x1E,0x16
2、从机到主机：
功能描述：发送数据
命令格式：
0xFe,0xFe,0x68,A0,Cmd,Len,Data,CS,0x16
...
＝0x90：历史数据，无后续数据包。
＝0x91：历史数据，有后续数据包。
        Len表示数据包的长度，这里固定为：＝15 字节。
        Data数据包： 测量值（ 3 byte ）×5
测量值 （ 3 byte ） ：测量值类型(1 byte ) + 数值（ 2 byte，高字节在前，低字节在后 ）
测量值类型(1 byte ) ：＝01：甲烷；＝02：氧气；＝03：一氧化碳；＝04：二氧化碳＝05：硫化氢。
    CS为校验和，计算0x68到Data部分的校验和（只发一个字节校验和码）。
        0x16为结束标志
3、通讯速率可调，可进行人工设置。
4、字节格式：
每字节含8 位二进制码，传输时加上一个起始位(0)、和一个停止位(1)，无奇偶校验位，共10 位。先传低位，后传高位。
